...
func main() {
	var (
		hostF = flag.String("host", "host", "Server host (valid values: host)")
	...
	)
	{
		...
		if addr == "" {
			...
			default:
				fmt.Fprintf(os.Stderr, "invalid host argument: %q (valid hosts: host)\n", *hostF)
				os.Exit(1)
			}
		}
		timeout = *timeoutF
...
Usage:
    ...
    -host HOST:  server host (host). valid values: host
